...
class Database
{
  ...
  public static function allWhere($table, $field, $value, $column_order = "id")
  {
    $query = self::connectionInit()->prepare("SELECT * FROM `" . $table . "` WHERE `" . $field . "` = :" . $field . " ORDER BY `" . $column_order . "`");
    $query->execute([$field => $value]);
    return $query->fetchAll(PDO::FETCH_OBJ);
  }
